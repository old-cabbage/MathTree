...
var FileOperations = class {
   ...
   * 实际的文件处理逻辑（私有方法）
   */
  async _getFilesToProcess(rules) {
      ...
      return rules.some((rule) => {
        if (rule.startsWith("/")) {
          ...
          if (!fileName.startsWith(".")) {
            ...
            patterns2.push(dirPath + "." + fileName);
          }
          const matched2 = patterns2.some((p) => (0, import_minimatch.default)(fileInfo.path, p, {
            dot: true,
            nocase: true,
            matchBase: false,
            noglobstar: false
          }));
          if (matched2 && this.DEBUG) {
            this.debug(`\u7EDD\u5BF9\u8DEF\u5F84\u5339\u914D\u6210\u529F - \u8DEF\u5F84: ${fileInfo.path}, \u89C4\u5219: ${rule}`);
          }
          return matched2;
        }
        const createPatterns = (pattern) => {
          if (pattern.startsWith(".")) {
            return [pattern];
          }
        ...
        if (rule.endsWith("/")) {
          ...
          const patterns2 = createPatterns(basePattern);
          const finalPatterns = patterns2.map((pattern) => {
            if (pattern.includes("*") && !pattern.startsWith("/")) {
              return "**/" + pattern;
            }
            return pattern;
          });
          const matched2 = fileInfo.isDirectory && finalPatterns.some(
            (pattern) => (0, import_minimatch.default)(fileInfo.path, pattern, {
              dot: true,
              nocase: true,
              matchBase: false,
              noglobstar: false
        ...
        const matched = !fileInfo.isDirectory && patterns.some(
          (pattern) => (0, import_minimatch.default)(fileInfo.path, pattern, {
            dot: true,
            nocase: true,
            matchBase: false,
  ...
  async addDotPrefix(fileInfo, isAdd = true) {
    ...
    try {
      ...
      if (!import_fs.default.existsSync(oldFullPath)) {
        throw new Error(`\u627E\u4E0D\u5230\u6587\u4EF6: ${fileInfo.path}`);
      }
      if (import_fs.default.existsSync(newFullPath)) {
        throw new Error(`\u76EE\u6807\u6587\u4EF6\u5DF2\u5B58\u5728: ${newPath}`);
      }
      import_fs.default.renameSync(oldFullPath, newFullPath);
  ...
  async rollback() {
    ...
    const lastBatch = this.operations.reduce((acc, op) => {
      if (!acc.timestamp || Math.abs(op.timestamp - acc.timestamp) < 1e3) {
    ...
    for (const op of lastBatch.operations.reverse()) {
      ...
      try {
        ...
        import_fs.default.renameSync(newFullPath, oldFullPath);
  ...
  getDisplayPath(fileOrPath, isFolder) {
    const path2 = typeof fileOrPath === "string" ? fileOrPath : fileOrPath.path;
    const shouldAddSlash = isFolder != null ? isFolder : fileOrPath instanceof import_obsidian.TFolder;
    return shouldAddSlash ? path2 + "/" : path2;
  }
};
...
var FileIgnoreSettingTab = class extends import_obsidian3.PluginSettingTab {
  ...
  async updateDisplayAsync() {
    ...
    this.renderMatchedFilesList(listContainer, null, null);
  ...
  async display() {
    ...
    const hideButton = new import_obsidian3.ButtonComponent(buttonGroup).setButtonText(this.t.applyRules.button).setCta().setTooltip(this.t.applyRules.desc).onClick(async () => {
      hideButton.setDisabled(true).setButtonText(this.t.applyRules.button + "...");
      try {
        if (!this.plugin.fileOps)
          throw new Error(this.t.notice.settingsErrorInit);
        await this.plugin.applyRules(true);
        new import_obsidian3.Notice(this.t.applyRules.success);
      } catch (err) {
        console.error("[file-ignore]", err);
        new import_obsidian3.Notice(this.t.applyRules.error + err.message);
      } finally {
        hideButton.setDisabled(false).setButtonText(this.t.applyRules.button);
        this.updateDisplayAsync();
      }
    });
    const showButton = new import_obsidian3.ButtonComponent(buttonGroup).setButtonText(this.t.revertRules.button).setTooltip(this.t.revertRules.desc).onClick(async () => {
      showButton.setDisabled(true).setButtonText(this.t.revertRules.button + "...");
      try {
        if (!this.plugin.fileOps)
          throw new Error(this.t.notice.settingsErrorInit);
        await this.plugin.applyRules(false);
        new import_obsidian3.Notice(this.t.revertRules.success);
      } catch (err) {
        console.error("[file-ignore]", err);
        new import_obsidian3.Notice(this.t.revertRules.error + err.message);
      } finally {
    ...
    leftTitleContainer.createEl("p", {
      text: this.t.ignoreRules.title,
      // "Ignore Rules"
      cls: "setting-item-name file-ignore-panel-title"
    });
    const ruleActionsContainer = leftTitleContainer.createDiv("file-ignore-rule-actions");
    new import_obsidian3.ButtonComponent(ruleActionsContainer).setButtonText(this.t.ignoreRules.resetButton).setTooltip("Restore default rules").setClass("file-ignore-small-button").onClick(() => {
      this.rulesTextArea.setValue(DEFAULT_SETTINGS.rules);
      this.rulesTextArea.onChanged();
    });
    new import_obsidian3.ButtonComponent(ruleActionsContainer).setButtonText(this.t.ignoreRules.allFilesButton).setTooltip("Add all root files/folders to the rules").setClass("file-ignore-small-button").onClick(async () => {
      try {
        if (!this.plugin.localFs) {
          new import_obsidian3.Notice("Local file system not available.");
          return;
        }
        const rootItems = await this.plugin.localFs.listDirectory();
        ...
        const combinedRules = [.../* @__PURE__ */ new Set([...existingRules, ...newRules])];
        this.rulesTextArea.setValue(combinedRules.join("\n"));
        this.rulesTextArea.onChanged();
      } catch (error) {
        console.error("[file-ignore] Error adding all files:", error);
        new import_obsidian3.Notice("Failed to add all files.");
      }
    });
    ...
    this.rulesTextArea.onChange(this.saveSettings);
    ...
    this.updateMatchedFilesImmediate();
  }
  ...
  renderMatchedFilesList(container, matchedFiles, error) {
    container.empty();
    if (error) {
...
var FileIgnorePlugin = class extends import_obsidian5.Plugin {
  async onload() {
    try {
      ...
      const locale = import_moment2.default.locale();
      if (locale.startsWith("zh")) {
        if (locale === "zh-tw" || locale === "zh-hk") {
          this.t = locales["zh-TW"];
        } else {
  ...
  async applyRules(isAdd = true) {
    try {
      if (!this.fileOps) {
        throw new Error("FileOperations \u672A\u521D\u59CB\u5316");
      }
      const rules = this.settings.rules.split("\n").map((line) => line.trim()).filter((line) => line && !line.startsWith("#"));
      if (!rules.length) {
